...
WHERE YEAR(t1.Start_Date) BETWEEN 2022 AND 2024;
	...
	async function handleRequest(request) {
	  ...
	  try {
		...
		// Validate schema
		const schemaConfig = SCHEMA_CONFIG[schemaName];
		if (!schemaConfig) throw new Error(`Invalid schemaName: ${schemaName}`);
		const { schema, prompt } = schemaConfig;
		if (!schema.trim()) throw new Error(`Schema for ${schemaName} is empty`);

		// Prepare system prompt with schema
		const systemPrompt = `${prompt}\nSchema: ${schema}`;
		console.log(`Processing request for schema: ${schemaName}, content: ${content}`);
